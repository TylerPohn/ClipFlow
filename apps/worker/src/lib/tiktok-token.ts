...
export async function ensureFreshTikTokToken(accountId: string): Promise<string> {
  ...
  // If we don't know the expiry or the token is still valid, return it as-is
  if (
    ...
    if (!account.refreshToken) {
      // Mark the account so the UI can prompt the user to re-link
    ...
    if (!data.access_token) {
      ...
      // Mark account as expired so UI can prompt reconnect
      await prisma.platformAccount.update({
        where: { id: accountId },
        data: { tokenStatus: 'expired' },
      });
    ...
    // Also update the NextAuth Account table for consistency
